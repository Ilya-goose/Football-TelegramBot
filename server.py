...
async def games(message: Message):
    await message.answer('🎮 Выберите одну игру из представленных на клавиатуре...', reply_markup=ReplyKeyboardMarkup(
...
async def game2(message: Message, state: FSMContext):
    await message.answer(
        'Правила очень просты:\nВам будет предложен список футболистов, из которого вам предстоит выбрать самых дорогих.\nПротив вас будет играть бот, который тоже будет пытаться собрать команду как можно дороже.\nПобеждает тот, кто соберет команду дороже чем у опонента.\n\nНажмите ❇ ПРОДОЛЖИТЬ ❇ для начала игры...',
